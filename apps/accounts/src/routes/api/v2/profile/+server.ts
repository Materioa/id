...
export async function GET({ request }: RequestEvent) {
  try {
    ...
    // Check for active ban in abuse_moderation_rules
    let isBanned = false;
    let banReason = '';
    try {
      ...
      if (banRule) {
        isBanned = true;
        banReason = banRule.body || banRule.reason || 'Violation of platform terms and conditions';
      }
    } catch (err) {
      console.error('Failed to check moderation rule:', err);
    }
